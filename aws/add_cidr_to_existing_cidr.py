	...
	for rule in resp['SecurityGroups']:
		...
		for ingress in rule['IpPermissions']:
			if old_cidr in [x['CidrIp'] for x in ingress['IpRanges']]:
				print(ingress)
				ports.append([ingress.get('IpProtocol'), ingress.get('FromPort', '-1'),ingress.get('ToPort', '-1')]) 
		sg_metadata[sg_id] = ports

	for sg_id in sg_metadata.keys():
		ip_permissions =  [{'IpProtocol':x[0],'FromPort':x[1],'ToPort':x[2],'IpRanges':[{'CidrIp':new_cidr}]} for x in sg_metadata[sg_id]]
		print('CidrIp="{}",GroupId="{}",IpPermissions={}'.format(new_cidr, sg_id, ip_permissions))
		# for port in ports:		
...
def main():
	parser = argparse.ArgumentParser(description="add new cidr group to aws security groups with an existing cidr group")
